...
func looksLikeKiroAccountManagerItem(item map[string]any) bool {
	...
	for _, key := range []string{"accessToken", "refreshToken", "profileArn", "clientIdHash", "clientId", "clientSecret", "authMethod", "provider", "machineId"} {
		if _, ok := item[key]; ok {
			return true
		}
	}
...
func validateDataAccount(item DataAccount) error {
	...
	if isKiroAccountManagerDataAccount(item) &&
		...
		return errors.New("Kiro 导入需要 refreshToken")
	}
	if isKiroAccountManagerDataAccount(item) &&
