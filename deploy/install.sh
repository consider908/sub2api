...
# Sub2API 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/consider908/sub2api/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="consider908/sub2api"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/sub2api" ]; then
        ...
        "$INSTALL_DIR/sub2api" --version 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+-[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
Documentation=https://github.com/consider908/sub2api
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/sub2api" --version 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+-[0-9]+' || echo "unknown")
...
install_version() {
    ...
    if [ "$current_version" = "$target_version" ] || [ "$current_version" = "$target_product_version" ]; then
...
main() {
    ...
    case "${1:-}" in
        ...
        --help|-h)
            ...
            echo "  $0 install -v 0.1.140-3   # Install specific version"
            echo "  $0 upgrade                # Upgrade to latest"
            echo "  $0 upgrade -v 0.1.140-3   # Upgrade to specific version"
            echo "  $0 rollback 0.1.140-3     # Rollback to 0.1.140-3"
