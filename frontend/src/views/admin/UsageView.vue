<template>
  ...
  <ConfirmDialog
    ...
    :title="t('usage.errors.cleanupConfirmTitle')"
    :message="t('usage.errors.cleanupConfirmMessage')"
    :confirm-text="t('common.delete')"
    :cancel-text="t('common.cancel')"
    :danger="true"
    @confirm="confirmClearErrorLogs"
...
const loadAdminErrors = async () => {
  ...
  try {
    const resp = await listErrorLogs(buildAdminErrorLogParams(true))
...
const confirmClearErrorLogs = async () => {
  errorCleanupConfirmVisible.value = false
  try {
    const result = await clearErrorLogs(buildAdminErrorLogParams(false))
    appStore.showSuccess(t('usage.errors.cleanupSuccess', { count: result.deleted_rows }))
    errPage.value = 1
    await loadAdminErrors()
  } catch (error) {
    console.error('Failed to clear admin errors:', error)
    appStore.showError(t('usage.errors.cleanupFailed'))
  }
}
